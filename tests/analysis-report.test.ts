...
import { buildTeamAnalysisEnvelope } from "../src/analysis/report.ts";
import type { AnalysisSource } from "../src/analysis/types.ts";
import { renderAnalysisYaml } from "../src/output/analysis.ts";
...
describe("compact analysis report", () => {
  test("matches the permanent success envelope exactly", () => {
    ...
    const envelope = buildTeamAnalysisEnvelope({
      ...
      effectiveAsOf: "2026-10-01T00:00:00.000Z",
      canonicalTeam: "Florida State",
      eligibleLeagueGames: games,
      scheduledTeamGames: includedGames,
      includedTeamGames: includedGames,
      data: {
        schedule: source("/games", {}, games),
        playTypes: source("/plays/types", {}, [
          { id: 1, text: "Rush", abbreviation: "RUSH" },
          { id: 2, text: "Pass Reception", abbreviation: "PASS" },
        ]),
        weeklyPlays: [source("/plays", { week: 1 }, plays)],
        drives: source("/drives", {}, drives),
        playerStats: source(
          "/games/players",
          {},
          includedGames.map((game) => playerStats(game.id)),
        ),
        advancedStats: source("/stats/game/advanced", {}, advanced),
        havocStats: source("/stats/game/havoc", {}, havoc),
      },
    });

    expect(parse(renderAnalysisYaml(envelope))).toEqual({
      team: "Florida State",
      year: 2026,
